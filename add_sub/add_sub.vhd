...
begin
f0: fulladder port map(a=>a(0), b=>b(0), mode=>mode, carryin=>mode, carryout=>co0, sum=>result(0));
...
f3: fulladder port map(a=>a(3), b=>b(3), mode=>mode, carryin=>co2, carryout=>carry, sum=>result(3));
end add_sub4_arch;
...
begin
adder1: add_sub4 port map(mode=> mode, a=>a, b=>b, carry=>bin_carry, result=>pre);
carry <= dec_carry;
dec_carry <= bin_carry or and1_out or and2_out;
and1_out <= pre(2) and pre(3);
and2_out <= pre(1) and pre(3);
adder2: add_sub4 port map(mode=>'0', a=>pre, b(0)=>'0', b(1)=>dec_carry, b(2)=>dec_carry, b(3)=>'0', result=>result);
